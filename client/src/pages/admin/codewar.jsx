...
const AdminCodewar = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await axios.get(`${import.meta.env.VITE_BACKENDAPI}/admin/codewar`);
            if (!res.data.success) {
                return toast.error(res.data.message);
            }
            setParticipants(res.data.codewarData);
        }
        fetchData();
    }, [])
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-white">CodeWar Competition</h1>
                    <p className="text-gray-400 mt-2">Manage registered participants</p>
                </div>
